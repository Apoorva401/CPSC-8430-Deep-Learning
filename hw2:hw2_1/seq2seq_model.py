...
def split_sentence(sentence, word_dict, w2i):
...
def annotate(label_file, word_dict, w2i):
    ...
    for d in label:
        for s in d['caption']:
            s = split_sentence(s, word_dict, w2i)
...
class VideoCaptioningModels(nn.Module):
    def __init__(self, encoder, decoder):
        super(VideoCaptioningModels, self).__init__()
...
def train(model, epoch, loss_fn, parameters, optimizer, train_loader):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        # Add the current loss to batch_losses
...
def main():
    ...
    train_dataset = TrainData(label_file, files_dir, word_dict, w2i)
    ...
    model = VideoCaptioningModels(encoder=encoder, decoder=decoder)
    ...
    total_batch_losses = []  # Track total losses across all epochs
    for epoch in range(epochs_n):
        batch_losses = train(model, epoch+1, loss_fn, parameters, optimizer, train_dataloader) 
        total_batch_losses.extend(batch_losses)

    torch.save(model, "{}/{}.h5".format('SavedModel', 'modelApoorvaGaddam'))
